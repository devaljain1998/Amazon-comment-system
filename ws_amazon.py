...
class Review:
    ...
    def get_all_reviews(show_all_reviews, csv_writer):
        ...
        for reviews in soup.find_all('div', {'data-hook' : 'review'}):
            ...
            Review.reviews_list.append({'title': review.title, 'body': review.body, 'rating': review.rating})
        ...
        try:
            ...
            get_full_url = Review.get_url(url) + get_to_next_page
            print(get_full_url)
            # count = Review.count
            Review.count = Review.count+1
            print(f'\n Fetcing all the reviews from Page No :-{Review.count} \n')
            
            # if count != 5:
            Review.get_all_reviews(get_full_url, csv_writer)
    ...
    def open_amazon_url(url):
        ...
        see_all_review_url = Review.get_url(url) + review_link
        ...
        Review.get_all_reviews(see_all_review_url, csv_writer)
...
get_all_reviews = Review.open_amazon_url(url)
# open_amazon_url(url)
print('-----------------------------------------------------------------------------')
print(f'\n\n\n{Review.reviews_list}\n\n\n')
